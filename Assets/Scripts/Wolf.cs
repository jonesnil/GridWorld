...
public class Wolf : MonoBehaviour
{
    ...
    public void Setup(int x, int y, int health)
    {
        ...
        if (allWolves == null)
        {
            allWolves = new List<Wolf>();
        }

        allWolves.Add(this.GetComponent<Wolf>());
    }
    ...
    void MovePath()
    {
        if (!(_tilePos == currentGoal) && currentPath != null && currentPath.ContainsKey(_tilePos))
        {
            ...
            {
                GameEvents.InvokeWolfPositionChanged(_tilePos, move);

                TileManager.grassMap[move.x, move.y].occupied = true;
                TileManager.grassMap[move.x, move.y].wolf = true;
                TileManager.grassMap[_tilePos.x, _tilePos.y].occupied = false;
                TileManager.grassMap[_tilePos.x, _tilePos.y].wolf = false;
    ...
    public void Move(bool[] ignoredDirections)
    {
        ...
        if (moves.Count != 0)
        {
            ...
            TileManager.grassMap[move.x, move.y].wolf = true;
            TileManager.grassMap[_tilePos.x, _tilePos.y].occupied = false;
            TileManager.grassMap[move.x, move.y].wolf = false;
    ...
    bool AdjacentWolf(Vector2Int pos)
    {
